...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateDashboard() {
        try {
            ...
            // Update the timestamp element robustly
            if (lastUpdatedElement) {
                const now = new Date();
                let formattedTime = '';

                try {
                    // Ensure userTimeZone is a valid, non-empty string before using it
                    if (!userTimeZone || typeof userTimeZone !== 'string') {
                        throw new Error('Invalid user timezone provided.');
                    }
                    ...
                    // This will throw an error if the userTimeZone is not recognized by the browser
                    formattedTime = now.toLocaleTimeString('en-US', options);

                } catch (e) {
                    console.warn(`Could not format time for timezone "${userTimeZone}". Falling back to local time.`, e);
                    // Fallback to a simple local time format if the provided timezone is invalid
                    ...
                    formattedTime = `${now.toLocaleTimeString('en-US', fallbackOptions)} (Local)`;
                }
                
                lastUpdatedElement.textContent = `Last Update: ${formattedTime}`;
            }
